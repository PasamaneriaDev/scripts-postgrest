...
CREATE OR REPLACE FUNCTION ordenes_venta.item_disponible_despachar(p_item character varying)
 RETURNS TABLE(bodega character varying, cliente_ubic character varying, cliente character varying, numero_pedido character varying, fecha_pedido date, fecha_entrega date, item character varying, cantidad_pendiente numeric, cantidad_despacho numeric, es_dudoso character varying, status character varying, verifica_vencimiento text, tiene_vencimiento boolean, protestado boolean)
 LANGUAGE plpgsql
AS $function$
DECLARE
...
BEGIN
    ...
    FOR registro IN (SELECT p.*
        ...
        LOOP
            ...
            UPDATE ordenes_venta.exisubic e
            SET disponible = CASE
            ...
            UPDATE ordenes_venta.auxcomprometido a
            SET disponible   = CASE
