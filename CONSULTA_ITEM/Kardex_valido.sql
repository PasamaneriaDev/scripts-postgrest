...
BEGIN
    ...
    FOR registro IN (SELECT t.*,
        ...
        LOOP
            ...
            IF registro.tipo_movimiento::VARCHAR IN ('VTAS ALM', 'DEVO ALM')
                /* AND registro.bodega::VARCHAR = ANY (p_bodega)*/ THEN
            ...
            ELSIF registro.tipo_movimiento::VARCHAR IN ('VTAS MAY', 'DEVO MAY')
                /* AND registro.bodega::VARCHAR = ANY (p_bodega)*/ THEN
                wDocumento = registro.documento::varchar;
            ...
            ELSIF registro.tipo_movimiento::VARCHAR IN ('COMP EXT', 'DEVO EXT')
                /* AND registro.bodega::VARCHAR = ANY (p_bodega)*/ THEN
    ...
    RETURN QUERY
        ...
        FROM AuxKardex ak
        WHERE (Wlongitud = 0)
           OR ((Wlongitud > 0 AND (ak.bodini = ANY (p_bodega) OR ak.bodfin = ANY (p_bodega))));
    DROP TABLE IF EXISTS AuxKardex;
END;
$function$
;
